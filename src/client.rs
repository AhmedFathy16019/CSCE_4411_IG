...
fn send_request_to_servers(server_addresses: &Vec<SocketAddr>, data: &[u8]) {
    // initialize socket
...
fn send_request_to_server(server_addr: &SocketAddr, data: &[u8]) {
    let socket = UdpSocket::bind("0.0.0.0:0")
        .expect("Failed to bind UDP socket");
...
fn handle_response(server_reply: &ServerReply, socket: &UdpSocket, data: &[u8]) {
    match server_reply {
        ServerReply::Address(address) => {
            send_request_to_server(&address, data);
            let server_reply = read_response(socket);
...
fn main() {
    ...
    for i in 0..number_of_requests {
        ...
        send_request_to_servers(&server_addresses, &request);
